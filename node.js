...
app.post('/register-card', async (req, res) => {
    try {
        ...
        const existing = await notion.databases.query({
            ...
            filter: { 
                property: 'ID Tarjeta', 
                rich_text: { equals: cardId }  // Corregido: rich_text en lugar de text
            }
        });
        
        if (existing.results.length > 0) {
            return res.status(400).json({  // Código 400 para error de cliente
                success: false, 
        ...
        res.json({ 
            success: true,
            id: response.id  // Devuelve el ID creado para verificación
        });
...
app.post('/register-visit', async (req, res) => {
    try {
        ...
        const clientResponse = await notion.databases.query({
            ...
            filter: { 
                property: 'ID Tarjeta', 
                rich_text: { equals: cardId }  // Corregido: rich_text
            }
        });
...
async function enviarPremio(client, visitas) {
    try {
        ...
        // Configurar nodemailer (REEMPLAZAR CON TUS CREDENCIALES)
